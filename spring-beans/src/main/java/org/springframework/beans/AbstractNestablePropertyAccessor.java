...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	...
	protected void setPropertyValue(PropertyTokenHolder tokens, PropertyValue pv) throws BeansException {
		/*在了解 BeanWrapper 设置和获取获取属性值前，我们先了解一下 PropertyTokenHolder。
		PropertyTokenHolder 是 BeanWrapperImpl 的内部类，他很简单，定义了属性命名描述。
		一个能标识 Bean 的唯一名称（可能包含嵌套的路径），Bean 真实名称（不包含嵌套路径），访问集合属性的索引数组。
		此种需要了解 PropertyTokenHolder，他是一个内部简单类，定义了嵌套的名称、真实名称、对应的访问KEY三个属性。
		博文：https://www.iteye.com/blog/bodu-li-2157771
		通过tokens.keys判断是否有集合类的属性*/
		if (tokens.keys != null) {
			processKeyedProperty(tokens, pv);
		}
		else {
			//从这里进入，能看到setValue方法，进行了注入
